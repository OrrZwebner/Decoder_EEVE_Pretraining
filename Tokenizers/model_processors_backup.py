    ...
    def expand_tokenizer(self, learned_tokens: List[str], algorithm_name: str, 
            ...
            if processed_tokens:
                ...
                if new_tokens:
                    ...
                    # REPLACE: # ... (logging statements) ... WITH:
                    self.logger.info(f"âœ… Successfully added {num_added} new tokens")
                    ...
                    self.logger.warning("âš ï¸ All processed tokens already exist in vocabulary")
                    return self.original_tokenizer, 0, processed_tokens
            else:
                self.logger.warning("âŒ No tokens to add")
                # MODIFIED RETURN STATEMENT
...
class LlamaProcessor(BaseProcessor):
    ...
    - Uses Ä  tokens for spaces at word beginnings
    ...
    def process_tokens_for_model(self, tokens: List[str], algorithm: str) -> List[str]:
        ...
        - Remove â– prefixes from Unigram/SentencePiece tokens
        - Convert Ä  tokens to space + text format
        ...
        for token in tokens:
            ...
            if processed_token.startswith("Ä "):
...
class GemmaProcessor(BaseProcessor):
    ...
    - Uses â– tokens for word-initial markers
    ...
    def process_tokens_for_model(self, tokens: List[str], algorithm: str) -> List[str]:
        ...
        - For other algorithms: create both â–prefixed (word-initial) and 
        ...
        for token in tokens:
            if "sentencepiece" in algorithm.lower():
                ...
                base_token = token.replace("##", "").replace("â–", "").strip()
                
                if len(base_token) > 0:
                    # Add word-initial version (with â– prefix)
                    prefixed_token = f"â–{base_token}"
        ...
        self.logger.info(f"Gemma processing: {len(tokens)} â†’ {len(processed_tokens)} tokens")
        return processed_tokens
...
class Gpt2Processor(BaseProcessor):
    ...
    def process_tokens_for_model(self, tokens: List[str], algorithm: str) -> List[str]:
        ...
        for token in tokens:
            ...
            if processed_token.startswith("â–"):
                processed_token = processed_token[1:]
                
            # Handle Ä  tokens (convert to space + text)
            if processed_token.startswith("Ä "):
                processed_token = " " + processed_token[1:]
        ...
        self.logger.info(f"GPT-2 processing: {len(tokens)} â†’ {len(processed_tokens)} tokens")
        return processed_tokens
...
def convert_unigram_to_bpe(tokens: List[str]) -> List[str]:
    """
    Convert Unigram tokens to BPE format by removing â– prefixes.
    
    Args:
        tokens (List[str]): Unigram tokens with â– prefixes
    
    Returns:
        List[str]: BPE-compatible tokens without prefixes
    """
    return [token.replace("â–", "") for token in tokens if len(token.replace("â–", "").strip()) > 0]


def convert_to_sentencepiece_format(tokens: List[str]) -> List[str]:
    """
    Convert tokens to SentencePiece format with appropriate â– prefixes.
    
    Creates both word-initial (â–prefixed) and continuation (non-prefixed) versions
    ...
    for token in tokens:
        ...
        if len(base_token) > 0:
            ...
            converted_tokens.append(f"â–{base_token}")
...
def handle_space_tokens(tokens: List[str], target_format: str = "llama") -> List[str]:
    ...
    - LLaMA: Uses Ä  prefix for word-initial spaces
    - Gemma/SentencePiece: Uses â– prefix for word boundaries
    - GPT-2: Uses Ä  prefix for word-initial spaces (similar to LLaMA)
    ...
    for token in tokens:
        ...
        if target_format.lower() in ["llama", "gpt2"]:
            # Convert â– to Ä  for LLaMA/GPT-2 compatibility
            if token.startswith("â–"):
                processed_token = "Ä " + token[1:]
            # Convert existing Ä  tokens to space + text
            elif token.startswith("Ä "):
                processed_token = " " + token[1:]
                
        elif target_format.lower() == "gemma":
            # Convert Ä  to â– for Gemma compatibility
            if token.startswith("Ä "):
                processed_token = "â–" + token[1:]
            # Ensure â– prefix for word-initial tokens
            elif not token.startswith("â–") and len(token.strip()) > 0:
                # Add â– prefix if it looks like a word-initial token
                if token[0].isalpha() or ord(token[0]) > 127:  # Unicode characters
                    processed_token = "â–" + token
